...
class WrapperRelative(gym.Env):
    def __init__(self, env):
        ...
        self.env = env
        spaces_dic = {
            'goal': spaces.Box(low=np.array([-7, -4]), high=np.array([7, 4]), dtype=np.int32),
            'lava': spaces.Box(low=np.array([-7, -4]), high=np.array([7, 4]), dtype=np.int32),
        }
    ...
    def getDistanceLava(self,board,reward):
        ...
        return  lavaState[1] - playerState[1],lavaState[0] - playerState[0]
    ...
    def getDistanceGoal(self,board):
        ...
        return    goalState[1] - playerState[1],goalState[0] - playerState[0]
