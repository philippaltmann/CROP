...
import numpy as np
import random
class BasicWrapperAugmentStack(gym.Env):
    def __init__(self, env):
        super(BasicWrapperAugmentStack, self).__init__()
        self.env = env
        spaces_dic = {
            'normal': spaces.Box(low=0,high=4,shape=(5,5),dtype=np.int32),
            'cut': spaces.Box(low=-1,high=4,shape=(5,5),dtype=np.int32),
            'crop': spaces.Box(low=0,high=4,shape=(5,5),dtype=np.int32),
            'rotate': spaces.Box(low=0,high=4,shape=(5,5),dtype=np.int32),
            'flip': spaces.Box(low=0,high=4,shape=(5,5),dtype=np.int32),
        }
    ...
    def get_observations(self,timestep):
        normal = self.modifyBoard(timestep.observation['board'])

        j = random.randint(1, 2)
        flip = []
        if (j == 1):
            flip = self.flip(self.modifyBoard(timestep.observation['board']), "x")
        if (j == 2):
            flip = self.flip(self.modifyBoard(timestep.observation['board']), "y")

        j = random.randint(1, 3)
        rotate = []
        if (j == 1):
            rotate = self.rotate(self.modifyBoard(timestep.observation['board']), 90)
        if (j == 2):
            rotate = self.rotate(self.modifyBoard(timestep.observation['board']), 180)
        if (j == 3):
            rotate = self.rotate(self.modifyBoard(timestep.observation['board']), 270)

        cut = self.cut(self.modifyBoard(timestep.observation['board']))
        crop = self.crop(timestep.observation['board'])
