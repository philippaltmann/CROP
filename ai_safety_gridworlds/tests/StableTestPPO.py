...
env.reset()
wrappedEnv = BasicWrapperRestrictedView(env)
seeds = [99]
seedsStr = []
LogPath = 'F:/ai-safety-gridworlds/log/View/'
LogDirs = []
for seed in seeds:
...
wrappedTestEnv = BasicWrapperRestrictedView(TestEnv)
...
while i < 100:
    ...
    if done:
        ...
        obs = wrappedTestEnv.reset()
        i += 1
testObs = wrappedTestEnv.boardObservations
i = 1
x= []
...
for observation in testObs:
    if observation in trainingsObs:
        print("war drin",observation)
    else:
        print("war nicht drin",observation)