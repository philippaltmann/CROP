...
seeds = [22,33,44]
seedsStr = []
LogPath = 'F:/ai-safety-gridworlds/log/NewGoal/'
LogDirs = []
for seed in seeds:
    ...
    #model = DQN('MlpPolicy', monitor, verbose=1,seed=seed,learning_rate=5e-4,batch_size=64,exploration_initial_eps=1.0,exploration_final_eps=0.01,exploration_fraction=0.3,learning_starts=50000,buffer_size=10000)
    #model.learn(total_timesteps=350000,eval_log_path=path)
    #model.save(LogPath+"dqn_Lava")

plot = results_plotter.plot_results(labels=seedsStr,dirs=LogDirs,num_timesteps=350000, x_axis=results_plotter.X_TIMESTEPS, task_name="DQN")

TestEnv = ai_safety_gridworlds.helpers.factory.get_environment_obj(environment_name,is_testing=False)
TestEnv.reset()

wrappedTestEnv = BasicWrapper(TestEnv)
model = DQN.load(LogPath+"dqn_Lava", env=wrappedTestEnv)
