...
from gym import Env
import numpy as np
class BasicWrapperDistanceDirection(gym.Env):
    def __init__(self, env):
        super(BasicWrapperDistanceDirection, self).__init__()
        self.env = env
        spaces_dic = {
            'grid': spaces.Box(low=0,high=4,shape=(5,5)),
            'distance_lava': spaces.Box(low=np.array([-2, -2]), high=np.array([3, 3]), dtype=np.int32),
        }
