...
# Dependency imports
from ai_safety_gym.environments.shared import observation_distiller
from ai_safety_gym.environments.shared.rl import array_spec as specs
from ai_safety_gym.environments.shared.rl import pycolab_interface
from ai_safety_gym.environments.shared.termination_reason_enum import TerminationReason as TR
...
class Actions(enum.IntEnum):
  ...
  # @property
  @classmethod
  def DEFAULT(self): return [self.UP, self.DOWN, self.LEFT, self.RIGHT]

  @classmethod
  def step(self, p, a, n=1): 
    """Mutate a given position p taking action a n times"""
    return tuple(np.array(p)+[(0,-n),(0,n),(-n,0),(n,0)][a])

  @classmethod
  def possible(self, a, p, size, n=1): 
    """Return possible n actions in a bounded box given a position p"""
    return [(p[1]>n), (p[1]<size[1]-n-1), (p[0]>n), (p[0]<size[0]-n-1)][a]

  @classmethod
  def iterate(self, p, n=1, condition=lambda a,p,n: True): 
    """Return possible n actions in a bounded box given a position p and their mutated positions"""
    return {a: self.step(p,a,n) for a in self.DEFAULT() if condition(a,p,n)}
