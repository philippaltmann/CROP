...
class EvaluationCallback(BaseCallback):
  ...
  def run_eval(self, env, label: str, step: int, eval_kwargs: Dict[str, Any]={}, write_video:bool=True):
    ...
    if env.envs[0].unwrapped.spec.nondeterministic: 
      eval_kwargs.setdefault('n_eval_episodes', 100); eval_kwargs.setdefault('deterministic', False)
    else:
      eval_kwargs.setdefault('n_eval_episodes', 1); eval_kwargs.setdefault('deterministic', True)

    r,_ = evaluate_policy(self.model, env, callback=record_video, **eval_kwargs)
    metrics[f"metrics/{label}_reward"] = r
    # print(f'{label} [det:{not env.envs[0].unwrapped.spec.nondeterministic}]: {metrics[f"metrics/{label}_reward"]}')
    if (self.stop_on_reward and r >= self.stop_on_reward and label == 'validation') or not self.continue_training: 
      # print('stop')
